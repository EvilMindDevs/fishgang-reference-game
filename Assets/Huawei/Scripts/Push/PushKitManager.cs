...
namespace HmsPlugin
{
    public class PushKitManager : MonoBehaviour, IPushListener
    {
        ...
        void Start()
        {
            ...
            Debug.Log($"[PushKitManager] Push token from GetToken is {token}");
            if (token != null)
            {
        ...
        {
            Debug.Log($"[PushKitManager] Push token from OnNewToken is {token}");
            if (token != null)
            {
                OnTokenSuccess?.Invoke(token);
            }
        ...
        {
            Debug.Log("PushKitManager Error asking for Push token");
